<template>
  <div class="text-xl flex flex-row h-full overflow-hidden text-[var(--color-text)]">
    <div
      class="h-auto flex flex-col w-half w-[35%] bg-foreground p-3.5 m-2.5 rounded-lg shadow-[var(--shadow-subtle)]"
    >
      <select
        v-model="format"
        class="w-full self-center text-2xl bg-foreground pb-1 mb-2 pt-1 rounded-lg shadow-[var(--shadow-subtle)] content-center text-center"
      >
        <option v-for="f in formats" :key="f" :value="f">Convert to {{ f }}</option>
      </select>
      <!--div class="bg-foreground p-3.5 mt-2.5 rounded-lg shadow-[var(--shadow-subtle)] mb-2 text-center  cursor-pointer">
        <div class="text-2xl w-full flex items-center cursor-pointer justify-center" @click="extraSettings = !extraSettings">
          <span>Advanced Settings</span>
          <ArrowDown :size="32" :class="{ 'rotate-90': extraSettings }" />
        </div>     
        <div v-if="extraSettings">
          <div v-for="obj in advancedSettings[format]" class="bg-foreground p-3.5 mt-2.5 rounded-lg shadow-[var(--shadow-subtle)] mb-2">
    ...
    <div class="w-[65%] mt-4 h-full overflow-y-auto">
      ...
      <div v-if="convertedUrls.length" class="flex flex-col">
        <button
          @click="downloadZip"
          class="cursor-pointer self-center text-center text-4xl bg-foreground p-3.5 mt-2.5 rounded-lg shadow-[var(--shadow-subtle)]"
        >
          Download All
        </button>
        <div class="p-4 grid grid-cols-4 gap-4">
          <a
            ...
            :key="index"
            class="bg-foreground p-3.5 mt-2.5 rounded-lg shadow-[var(--shadow-subtle)] flex flex-row flex-wrap basis-full order-last"
            :href="img.url"
            :download="img.fileName.split('.')[0] + '_converted.' + format.toLowerCase()"
          >
            <img :src="img.url" :alt="`Converted as ${format}`" class="max-w-full" />
            <span class="mt-2 text-blue-600 underline text-center w-full"> Download </span>
          </a>
        </div>
      </div>
    </div>
...
const advancedSettings: Partial<
  Record<MagickFormat, Record<string, { min: number; reference: Ref; max: number; name: string }>>
> = {
  JPG: {
    quality: {
      min: 1,
...
async function downloadZip() {
  ...
  convertedUrls.value.forEach((item, index) => {
    const fileName =
      item.fileName.split('.')[0] + '.' + format.value.toLowerCase() || `file${index + 1}.bin`
